...
void stdio_canvas() {
    ...
    int h, c;
    //const char* str="Hello World (press u, d, ↑, or ↓)";  // window con doesn't yet support unicode (arrows are UTF-8)
    const char* str="Hello World (u, d, <UpArrow>, or <DnArrow>)";
    const char* spc="                                                   ";
	 char inbuf[6];  // UTF should be max 4 bytes plus string ending 0
    ...
    while (1) {
		...
		// an alternative to io_get_l, it to use getchar(), io_getc(), getc(stdin), fgetc(stdin), which all return int unicode codepoint
